...
def send_notification(message):
    # TODO: LINE NotifyやGmail APIの設定をここに書く
    # （現状はターミナルに表示するだけのダミー実装）
    print("="*40)
    print("🔔 [通知] \n" + message)
    print("="*40)
...
def assign_seat(date, time, people):
    ...
    confirmed = [
        ...
        if r.get("date") == date and r.get("time") == time and r.get("status") == "確定"
    ]
    ...
    if 1 <= people <= 2:
        if counter_used + people <= 10:
            return "確定", "counter"
    elif 3 <= people <= 4:
        if table4_used < 4:
            return "確定", "table4"
        elif zashiki4_used < 3:
            return "確定", "zashiki4"
    elif 5 <= people <= 6:
        if zashiki6_used < 1:
            return "確定", "zashiki6"
...
def index():
    ...
    if request.method == "POST":
        ...
        for r in reservations:
            if r.get("name") == name and r.get("phone") == phone and r.get("date") == date and r.get("time") == time:
                error_msg = "同じ日時で既に予約が入っています。"
                break
        ...
        if not error_msg:
            ...
            # 予約完了通知
            send_notification(f"【新規予約】{name}様 {people}名 ({date} {time}) コース:{course}")
...
def cancel(id):
    ...
    if data and data.get("date", "") > today_str:
        ...
        send_notification(f"【キャンセル】{data.get('name')}様 ({data.get('date')} {data.get('time')})")
